...
class ElevatorController:
    ...
    def bind_buttons(self):
        for floor, button in self.app.buttons.items():
            button.configure(command=lambda f=floor: self.add_request(f))
    ...
    def add_request(self, floor):
        ...
        self.update_debug_terminal()  # uppdatera debug 
    ...
    def run(self):
        while self.running:
            if not self.requests:
                self.update_debug_terminal()  # fixar tom kö i debug
                time.sleep(0.2)
                continue
    ...
    def get_next_floor_scan(self):
        ...
        higher = sorted([f for f in self.requests if f > current])
        lower = sorted([f for f in self.requests if f < current], reverse=True)
    ...
    def move_to_floor(self, target):
        while self.app.current_floor != target:
            ...
            self.update_debug_terminal(target=target)  # uppdatera debug under rörelse
    ...
    def stop_at_floor(self):
        ...
        self.update_debug_terminal()  # visar borttaget mål i debug
        time.sleep(3)  # för tid på våning
